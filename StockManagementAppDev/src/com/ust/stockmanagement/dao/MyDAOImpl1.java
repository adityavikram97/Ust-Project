...
public class MyDAOImpl1 implements MyDAO{
	...
	public List<ProductDTO> addtocart(int id){
		...
		ProductDTO l=ss.load(ProductDTO.class,id);   
	     ...
	     }else if (l.getCategory().equalsIgnoreCase("cloths")) {
	    	 totalpricewithgst=price*0.05+price;
		}else if (l.getCategory().equalsIgnoreCase("Pantry")) {
	     ...
	     int quantity =l.getQuantity();
		    ...
		    OrderDTO odto=new OrderDTO();
		     odto.setTotal_price(price);
		      odto.setTotal_price_with_gst(totalpricewithgst);
		      odto.setPlist(plist);
		      ss.save(odto);
		      ss.beginTransaction().commit();
